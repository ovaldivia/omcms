...
var searchreplace = (function () {
  ...
  var $_78g06iisjf3ft2wo = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_78g06iisjf3ft2wo.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_50p2tliqjf3ft2wf.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_50p2tliqjf3ft2wf.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_50p2tliqjf3ft2wf.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_50p2tliqjf3ft2wf.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_50p2tliqjf3ft2wf.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_50p2tliqjf3ft2wf.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_50p2tliqjf3ft2wf.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_50p2tliqjf3ft2wf.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_50p2tliqjf3ft2wf.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_50p2tliqjf3ft2wf.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_50p2tliqjf3ft2wf.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_50p2tliqjf3ft2wf.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_50p2tliqjf3ft2wf.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_50p2tliqjf3ft2wf.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_50p2tliqjf3ft2wf.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_83vhjbiujf3ft2xf = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_83vhjbiujf3ft2xf.open(editor, currentIndexState);
    });
  };
  var $_poadfitjf3ft2xc = { register: register };
  ...
  var $_1fua8uivjf3ft2xl = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_poadfitjf3ft2xc.register(editor, currentIndexState);
    $_1fua8uivjf3ft2xl.register(editor, currentIndexState);
    return $_ctlxahipjf3ft2wc.get(editor, currentIndexState);
  });
  function Plugin () {
  }
